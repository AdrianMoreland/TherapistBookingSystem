...
router.get("/", isLoggedIn, async (req, res) => {
  ...
  const { username } = req.user; // get email/username from req.user property created by isLoggedIn middleware
...
router.get("/:id", isLoggedIn, async (req, res) => {
  const { Todo } = req.context.models;
  const { username } = req.user; // get email from req.user property created by isLoggedIn middleware
...
router.post("/", isLoggedIn, async (req, res) => {
  const { Todo } = req.context.models;
  const { username } = req.user; // get email from req.user property created by isLoggedIn middleware
  req.body.username = username; // add email property to req.body
